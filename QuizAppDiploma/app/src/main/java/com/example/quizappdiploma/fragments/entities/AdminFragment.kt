...
class AdminFragment : Fragment() {
    ...
    private val usersFragment = UserListFragment()
    ...
    private val tabButtons by lazy {
        listOf(binding.tabUsers, binding.tabCourses, binding.tabLectures, binding.tabQuestions)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.nameTextView.text = user?.username ?: "Admin"
        binding.emailTextView.text = user?.email ?: ""

        binding.tabUsers.setOnClickListener { selectTab(0) }
        binding.tabCourses.setOnClickListener { selectTab(1) }
        binding.tabLectures.setOnClickListener { selectTab(2) }
        binding.tabQuestions.setOnClickListener { selectTab(3) }

        binding.logoutButton.setOnClickListener {
            preferenceManager.logout()
            findNavController().navigate(R.id.action_adminFragment_to_welcomeFragment)
        }

        binding.importButton.setOnClickListener {
            findNavController().navigate(R.id.action_adminFragment_to_importContentFragment)
        }
    ...
    private fun selectTab(index: Int) {
        val fragments = listOf(usersFragment, coursesFragment, lecturesFragment, questionsFragment)
